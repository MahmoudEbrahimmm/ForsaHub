            ...
            <div class="col-md-8 m-auto">
                ...
                <div class="bg-white p-3 rounded shadow-sm">
                    <form action="{{ $formAction }}" method="POST">
                        ...
                        <label class="mb-2 mt-3">Address</label>
                        ...
                        <input type="text" name="industry" class="form-control @error('industry') is-invalid @enderror"
                            value="{{ old('industry', $company ? $company->industry : '') }}">
                        @error('industry')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        @if (auth()->user()->role == 'admin')
                            ...
                            <select name="owner_id" class="form-control @error('owner_id') is-invalid @enderror">
                                ...
                                @foreach ($owners ?? [] as $owner)
                                    <option value="{{ $owner->id }}" {{ old('owner_id', $company ? $company->owner_id : '') == $owner->id ? 'selected' : '' }}>
                                        {{ $owner->name }}
                                    </option>
                                @endforeach
                            </select>
